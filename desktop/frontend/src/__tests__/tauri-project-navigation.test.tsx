...
assert.ok(calls.some(call => call.name === "bridge_switch_session" && call.args.sessionId === "old-alpha"), "project switching skips the newest missing session");
...
assert.ok(create);
const bridgeCallsBeforeDraft = calls.filter(call => call.name === "bridge_switch_session").length;
const rememberedBeforeDraft = calls.filter(call => call.name === "remember_workbench_session").length;
await act(async () => { create.click(); await new Promise(resolve => setTimeout(resolve, 0)); });
assert.match(document.body.textContent ?? "", /把复杂的事/);
assert.equal(calls.filter(call => call.name === "bridge_switch_session").length, bridgeCallsBeforeDraft, "new chat stays local until Send");
assert.equal(calls.filter(call => call.name === "remember_workbench_session").length, rememberedBeforeDraft, "an empty draft is not stored");
assert.ok(![...document.querySelectorAll<HTMLButtonElement>(".tauri-sidebar__session")].some(button => button.textContent?.trim() === "新对话"), "an empty draft has no sidebar row");
...
assert.equal(calls.filter(call => call.name === "bridge_switch_session").length, bridgeCallsBeforeDraft, "typing in a draft does not open a session");
await act(async () => { create.click(); await new Promise(resolve => setTimeout(resolve, 0)); });
assert.equal(document.querySelector<HTMLTextAreaElement>(".tauri-composer textarea")?.value, "", "new chat discards an unsent draft");
assert.equal(calls.filter(call => call.name === "bridge_switch_session").length, bridgeCallsBeforeDraft, "repeated New still does not open a session");
assert.equal(calls.filter(call => call.name === "remember_workbench_session").length, rememberedBeforeDraft, "repeated New still stores no row");
await act(async () => {
  const textarea = document.querySelector<HTMLTextAreaElement>(".tauri-composer textarea");
  assert.ok(textarea);
  Object.getOwnPropertyDescriptor(dom.window.HTMLTextAreaElement.prototype, "value")!.set!.call(textarea, "修复侧栏显示问题");
...
assert.ok(calls.some(call => call.name === "bridge_switch_session" && call.args.sessionId === "tauri-stub-session" && call.args.workspaceRoot === "/work/beta"));
assert.ok(calls.some(call => call.name === "bridge_submit" && call.args.sessionId === "tauri-stub-session"));
assert.ok(calls.some(call => call.name === "remember_workbench_session" && call.args.sessionId === "tauri-stub-session"), "first accepted send stores the session");
assert.ok(calls.filter(call => call.name === "workbench_session_page").length >= 4, "first send refreshes the identity-backed sidebar");
...
await act(async () => {
  ...
  const textarea = document.querySelector<HTMLTextAreaElement>(".tauri-composer textarea");
  assert.ok(textarea);
  Object.getOwnPropertyDescriptor(dom.window.HTMLTextAreaElement.prototype, "value")!.set!.call(textarea, "触发目录重查");
  textarea.dispatchEvent(new dom.window.Event("input", { bubbles: true }));
  document.querySelector<HTMLButtonElement>('[aria-label="发送消息"]')?.click();
  await new Promise(resolve => setTimeout(resolve, 130));
  await new Promise(resolve => setTimeout(resolve, 0));
