...
隔离恢复用例另外验证 Preview SQLite 会话事件表、事件与 checkpoint 水位、事件 JSONL 投影和 transcript 一同进入快照；恢复到新的临时 profile 后逐项读回，并从恢复后的数据库成功追加后续事件。这仅验证离线 snapshot staging 的文件完整性与事件库可继续写入，不代表真实 profile 已完成恢复演练。
...
**候选旧 host 完整测试复测（2026-09-26）**：从 `50c1b9bc65ad6f95f4fef0d46dbad560d4827390` 归档旧 Tauri 源码，使用当前工作树构建的 Go bridge（HEAD `e2130095ef6fc3e894833978807b6a6bf3743d59`，含本地未提交改动；bridge SHA-256 `abfcfb47d5e67d4313ff813a4761400b6e4892cc3a7b9e973580c7f61ba77417`）。将旧 host 的完整 28 项 Rust 测试串行运行，包含真实 bridge 启停、意外退出恢复、标题重启回读和隔离会话删除，28 项全部通过；并构建候选 debug host，SHA-256 `b4896b10ed362726e721a81fecb8e7cda4cffb248defc0d4119b6432c3b1416b`。串行是必要的：旧测试并行修改进程级 profile 环境变量，会让 sidecar 启动测试互相干扰。产物和测试 profile 在 `/private/tmp/reasonix-tauri-oldhost-50c1b9bc6`，来自候选源码而非签名发布包；没有启动应用窗口，也未验证真实旧 Wails writer 停写，因此兼容矩阵结论仍不升级。

**工作树差异兼容复测（2026-09-26）**：复核发现 `verify-legacy-host.sh` 原先只归档 bridge 的 `HEAD`，会漏掉正在评审的未提交修改；脚本现先从 `HEAD` 构建可复现副本，再叠加 staged/unstaged diff 与非忽略的未跟踪文件，并输出 diff 与 sidecar 的 SHA-256。修正后立即以当前 bridge 工作树重跑旧 host 完整 28 项测试，全部通过；候选 debug host SHA-256 为 `d33b537b92d5a91248fd3216222277315a1be51dbb1c311229012927f8f70610`，当前 bridge SHA-256 为 `12b1842fb82fd8900a661cb28a61c22b687532fa36c0c51f77e58de0bdd47aa4`，隔离产物在 `/var/folders/j8/bqc5mc190_q6f4ytd9d32hlm0000gn/T/reasonix-tauri-compat.1uxAwV`。这认证的是旧 host 源码测试 harness 加当前未提交 bridge，不是签名发布包、实际旧 host 二进制或旧 Wails writer 停写。

**复测脚本全套运行（2026-09-26）**：`bash tools/tauri/verify-legacy-host.sh` 从旧 host `50c1b9bc6` 与当前 HEAD bridge `e2130095ef6fc3e894833978807b6a6bf3743d59` 的归档源码构建，串行运行完整 28 项旧 host 测试并通过，再构建候选 debug host，SHA-256 `be26a56eb9c014576c1401a9b15074bb98bdefc58de8aa77e996806efbbd8a14`；隔离产物与 profile 位于 `/var/folders/j8/bqc5mc190_q6f4ytd9d32hlm0000gn/T/reasonix-tauri-compat.tAlu3k`。这验证脚本可复现整套候选源码测试，不代表签名发布包或窗口操作认证。

1. 新会话先 `reserved` 登记 ID；实际写入后转 `ready`。bridge 的打开/切换/重命名/删除均先解析身份行，再定位文件。无记录的新建与已登记但 `missing` 的恢复必须分开。
2. 一段发布窗口内，host 对比 JSON 与 SQLite 的 ID/标题/工作区/顺序/文件状态；只记录差异统计，不把私密消息写诊断。若 ID、workspace、顺序和物理状态通过安全核验，title-only drift 与 inventory 确认的 missing identity rows 不阻止 SQLite 身份目录分页；UI 分别说明采用身份目录标题或标记文件缺失。其他可读差异或盘点错误下，若身份页与同轮 shadow projection 逐页匹配，则显示全量 `identity_unverified` 只读分页；身份页不匹配或首屏不可用时才回退到最多 50 条 JSON。shadow 审计本身不可用时，仍先尝试只读身份分页，身份首屏不可用才显示兼容回退。
